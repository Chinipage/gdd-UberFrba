...
PRINT 'UNIQUE FUNC_DESCRIPCION'
ALTER TABLE GESTION_DE_GATOS.FUNCIONALIDAD ADD CONSTRAINT UQ_FUNC_DESCRIPCION UNIQUE(FUNC_DESCRIPCION)
PRINT 'UNIQUE ROL_DESCRIPCION'
ALTER TABLE GESTION_DE_GATOS.ROL ADD CONSTRAINT UQ_ROL_DESCRIPCION UNIQUE(ROL_DESCRIPCION)
PRINT 'UNIQUE USUA_USERNAME'
ALTER TABLE GESTION_DE_GATOS.USUARIO ADD CONSTRAINT UQ_USUA_USERNAME UNIQUE(USUA_USERNAME)
PRINT '***************************************************************'
PRINT ''
...
BEGIN
	...
	IF NOT EXISTS
		...
		WHERE DATEPART(HOUR, @VIAJE_HORA_INICIO) >= TURN_INICIO AND DATEPART(HOUR, @VIAJE_HORA_INICIO) < TURN_FIN)
	BEGIN
		RAISERROR('No existe un turno para esa hora de inicio', 16, 1)
		RETURN
	END
	...
	IF 	NOT EXISTS
		(SELECT 1
		FROM GESTION_DE_GATOS.TURNO
		WHERE TURN_HABILITADO = 1 AND DATEPART(HOUR, @VIAJE_HORA_INICIO) >= TURN_INICIO AND DATEPART(HOUR, @VIAJE_HORA_INICIO) < TURN_FIN)
		AND
		EXISTS
		(SELECT 1
		FROM GESTION_DE_GATOS.TURNO
		WHERE TURN_HABILITADO = 0 AND DATEPART(HOUR, @VIAJE_HORA_INICIO) >= TURN_INICIO AND DATEPART(HOUR, @VIAJE_HORA_INICIO) < TURN_FIN)
	BEGIN
	...
	IF NOT EXISTS
		...
		WHERE VC_CHOF_ID = @CHOFER AND VC_TURN_ID = @TURN_ID)
	BEGIN
		RAISERROR('El chofer no puede tomar viajes en ese horario, no tiene el turno asignado', 16, 1)
		RETURN
	END
	...
	WHERE
		CHOF_ID = @CHOFER AND
		DATEPART(HOUR, @VIAJE_HORA_INICIO) >= TURN_INICIO AND DATEPART(HOUR, @VIAJE_HORA_INICIO) < TURN_FIN
END
GO
...
AS
	...
	BEGIN CATCH
		...
		IF @NUMBER = 2627 AND @MESSAGE LIKE '%UQ_VEHI_PATENTE%'
		BEGIN
			SET @MESSAGE = 'Ya existe un vehiculo con esa patente'
		END
...
AS
	...
	BEGIN CATCH
		...
		IF @NUMBER = 2627 AND @MESSAGE LIKE '%UQ_CLIE_DNI%'
		BEGIN
			SET @MESSAGE = 'Ya existe un cliente con ese DNI'
		END

		IF @NUMBER = 2627 AND @MESSAGE LIKE '%UQ_CLIE_TEL%'
		BEGIN
			SET @MESSAGE = 'Ya existe un cliente con ese telefono'
		END

		IF @NUMBER = 2627 AND @MESSAGE LIKE '%UQ_USUA_USERNAME%'
		BEGIN
			SET @MESSAGE = 'Ya existe un usuario con ese DNI'
		END
...
PRINT 'Procedimiento modificar_cliente'
GO
CREATE PROCEDURE GESTION_DE_GATOS.p_modificar_cliente
	@CLIE_ID INT,
	@NOMBRE VARCHAR(255),
	@APELLIDO VARCHAR(255),
	@DNI NUMERIC(18,0),
...
AS
	...
	BEGIN TRY
		...
		--Se asocia el rol chofer al nuevo chofer
		INSERT GESTION_DE_GATOS.ROL_USUARIO (RU_ROL_ID, RU_USUA_ID)
	...
	BEGIN CATCH
		...
		IF @NUMBER = 2627 AND @MESSAGE LIKE '%UQ_CHOF_DNI%'
		BEGIN
			SET @MESSAGE = 'Ya existe un chofer con ese DNI'
		END

		IF @NUMBER = 2627 AND @MESSAGE LIKE '%UQ_CHOF_TEL%'
		BEGIN
			SET @MESSAGE = 'Ya existe un chofer con ese telefono'
		END
...
PRINT 'Procedimiento modificar_chofer'
GO
CREATE PROCEDURE GESTION_DE_GATOS.p_modificar_chofer
	@CHOF_ID INT,
	@NOMBRE VARCHAR(255),
	@APELLIDO VARCHAR(255),
	@DNI NUMERIC(18,0),
...
AS
	BEGIN TRANSACTION
	BEGIN TRY
		--Se modifica el chofer
		UPDATE GESTION_DE_GATOS.CHOFER
		SET
			CHOF_NOMBRE = @NOMBRE,
			CHOF_APELLIDO = @APELLIDO,
			CHOF_DIRECCION = @DIRECCION,
			CHOF_DNI = @DNI,
			CHOF_FECHA_NACIMIENTO = @FECHA_NACIMIENTO,
			CHOF_MAIL = @MAIL,
...
BEGIN
	IF EXISTS
		(SELECT 1
		FROM GESTION_DE_GATOS.TURNO turn JOIN inserted ins ON ins.TURN_ID <> turn.TURN_ID AND ins.TURN_HABILITADO = 1 AND turn.TURN_HABILITADO = 1
		WHERE
			ins.TURN_INICIO = ins.TURN_FIN OR
			ins.TURN_INICIO > ins.TURN_FIN OR
			(ins.TURN_INICIO BETWEEN turn.TURN_INICIO AND turn.TURN_FIN - 1 OR ins.TURN_FIN BETWEEN turn.TURN_INICIO + 1 AND turn.TURN_FIN) OR
			(turn.TURN_INICIO BETWEEN ins.TURN_INICIO AND ins.TURN_FIN - 1 OR turn.TURN_FIN BETWEEN ins.TURN_INICIO + 1 AND ins.TURN_FIN)
		)
	BEGIN
...
BEGIN
	...
	IF EXISTS
		(SELECT 1
		FROM inserted
		WHERE VIAJ_FECHA_INICIO > GETDATE() OR VIAJ_FECHA_FIN > GETDATE()
		)
	BEGIN
	...
	IF EXISTS
		(SELECT 1
		FROM GESTION_DE_GATOS.CHOFER JOIN inserted ON CHOF_ID = VIAJ_CHOFER
	...
	IF EXISTS
		(SELECT 1
		FROM GESTION_DE_GATOS.TURNO turn JOIN inserted ins ON turn.TURN_DESCRIPCION = ins.VIAJ_TURN_DESCRIPCION
		WHERE turn.TURN_HABILITADO = 0
		)
	BEGIN
	...
	IF EXISTS
		(SELECT 1
		FROM GESTION_DE_GATOS.CLIENTE JOIN inserted ON CLIE_ID = VIAJ_CLIENTE
	...
	IF EXISTS
		...
		WHERE
			CAST(ins.VIAJ_FECHA_INICIO AS TIME) = CAST(viaj.VIAJ_FECHA_INICIO AS TIME) AND CAST(ins.VIAJ_FECHA_FIN AS TIME) = CAST(viaj.VIAJ_FECHA_FIN AS TIME) OR
...
PRINT 'Trigger cliente'
GO
CREATE TRIGGER GESTION_DE_GATOS.t_cliente ON GESTION_DE_GATOS.CLIENTE
AFTER UPDATE
AS
BEGIN
	BEGIN TRY
		--Se modifica el usuario del cliente en caso de cambiar el dni
		UPDATE GESTION_DE_GATOS.USUARIO
		SET
			USUA_USERNAME = ins.CLIE_DNI,
			USUA_CONTRASENIA = (SELECT GESTION_DE_GATOS.f_encriptar_contrasenia(ins.CLIE_DNI))
		FROM inserted ins
		WHERE USUA_ID = ins.CLIE_USUARIO
	END TRY
	BEGIN CATCH
		DECLARE
		@MESSAGE NVARCHAR(4000),
...
BEGIN
	BEGIN TRY
		--Se modifica el usuario del chofer en caso de cambiar el dni
		UPDATE GESTION_DE_GATOS.USUARIO
		SET
			USUA_USERNAME = ins.CHOF_DNI,
			USUA_CONTRASENIA = (SELECT GESTION_DE_GATOS.f_encriptar_contrasenia(ins.CHOF_DNI))
		FROM inserted ins
		WHERE USUA_ID = ins.CHOF_USUARIO
