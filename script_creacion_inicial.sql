...
CREATE TABLE GESTION_DE_GATOS.FACTURACION_VIAJE (
	...
	FV_TURN_VALOR_KILOMETRO NUMERIC(18,2) NOT NULL,
	FV_TURN_PRECIO_BASE NUMERIC(18,2) NOT NULL
)
...
AS
	...
	SELECT
		CHOF_ID,
		ISNULL(VIAJ_ID, -1) VIAJ_ID,
		ISNULL (RV_IMPORTE, 0) RV_IMPORTE,
		REND_IMPORTE
	FROM GESTION_DE_GATOS.RENDICION
		JOIN GESTION_DE_GATOS.CHOFER ON REND_CHOFER = CHOF_ID
		LEFT JOIN GESTION_DE_GATOS.RENDICION_VIAJE ON REND_ID = RV_REND_ID
		LEFT JOIN GESTION_DE_GATOS.VIAJE ON RV_VIAJ_ID = VIAJ_ID
...
AS
	...
	BEGIN TRY
		...
		IF EXISTS(
			SELECT 1
			FROM GESTION_DE_GATOS.VIAJE
			WHERE CAST(VIAJ_FECHA_FIN AS DATE) = @FECHA_FIN
	...
	SELECT
		CLIE_ID,
		ISNULL(VIAJ_ID, -1) VIAJ_ID,
		ISNULL(FV_IMPORTE, 0) FV_IMPORTE,
		FACT_IMPORTE
	FROM GESTION_DE_GATOS.FACTURACION
		JOIN GESTION_DE_GATOS.CLIENTE ON FACT_CLIENTE = CLIE_ID
		LEFT JOIN GESTION_DE_GATOS.FACTURACION_VIAJE ON FACT_ID = FV_FACT_ID
		LEFT JOIN GESTION_DE_GATOS.VIAJE ON FV_VIAJ_ID = VIAJ_ID
