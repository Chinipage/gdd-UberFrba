...
CREATE FUNCTION GESTION_DE_GATOS.encriptar_contrasenia(@contrasenia VARCHAR(255))
RETURNS VARCHAR(255)
AS
BEGIN
...
AS
	...
	(
		SELECT CONVERT(VARCHAR(255), Chofer_Dni), (SELECT GESTION_DE_GATOS.encriptar_contrasenia(RTRIM(CONVERT(VARCHAR(255), Chofer_Dni)))), CAST(1 AS BIT)
		FROM gd_esquema.Maestra
		GROUP BY Chofer_Dni
	)
	...
	INSERT GESTION_DE_GATOS.USUARIO (USUA_USERNAME, USUA_CONTRASENIA, USUA_HABILITADO)
	(
		SELECT CONVERT(VARCHAR(255), Cliente_Dni), (SELECT GESTION_DE_GATOS.encriptar_contrasenia(RTRIM(CONVERT(VARCHAR(255), Cliente_Dni)))), CAST(1 AS BIT)
		FROM gd_esquema.Maestra
...
GO
CREATE VIEW GESTION_DE_GATOS.chof_recaudacion
AS
	SELECT
		CHOF_ID 'Chofer',
		CHOF_NOMBRE 'Nombre',
...
GO
CREATE VIEW GESTION_DE_GATOS.chof_viaje
AS
	SELECT
		CHOF_ID 'Chofer',
		CHOF_NOMBRE 'Nombre',
...
GO
CREATE VIEW GESTION_DE_GATOS.clie_consumo
AS
	SELECT
		CLIE_ID 'Cliente',
		CLIE_NOMBRE 'Nombre',
...
GO
CREATE VIEW GESTION_DE_GATOS.clie_vehi
AS
	SELECT
		CLIE_ID 'Cliente',
		CLIE_NOMBRE 'Nombre',
...
GO
CREATE PROCEDURE GESTION_DE_GATOS.rend_viajes @FECHA_FACTURACION DATE
AS
	DECLARE
		@ID_RENDICION INT,
		@CHOFER INT

	IF(@FECHA_FACTURACION > CURRENT_TIMESTAMP)
		RAISERROR('No se puede generar la rendicion de una fecha futura', 16, 1)

	DECLARE CHOFERES CURSOR FOR (
		SELECT CHOF_ID
		FROM GESTION_DE_GATOS.CHOFER
		WHERE CHOF_HABILITADO = 1
	)
	OPEN CHOFERES
	FETCH NEXT FROM CHOFERES INTO @CHOFER
	WHILE @@FETCH_STATUS = 0
	BEGIN
		INSERT GESTION_DE_GATOS.RENDICION (REND_FECHA, REND_IMPORTE, REND_CHOFER) VALUES (@FECHA_FACTURACION, 0, @CHOFER)
		...
		(
			SELECT
				VIAJ_ID,
				@ID_RENDICION,
				(TURN_PRECIO_BASE + TURN_VALOR_KILOMETRO * VIAJ_DISTANCIA) * 0.3
			FROM GESTION_DE_GATOS.VIAJE
				JOIN GESTION_DE_GATOS.TURNO ON VIAJ_TURNO = TURN_ID
...
--GO
--CREATE PROCEDURE GESTION_DE_GATOS.fact_viajes @FECHA_FACTURACION DATE
--AS
--	DECLARE
--		@ID_FACTURACION INT,
--		@CLIENTE INT
--
--	IF(@FECHA_FACTURACION > CURRENT_TIMESTAMP)
--		RAISERROR('No se puede generar la facturacion de una fecha futura',16,1)
--
--	DECLARE CLIENTES CURSOR FOR (
--		SELECT CLIE_ID
--		FROM GESTION_DE_GATOS.CLIENTE
--		WHERE CLIE_HABILITADO = 1
--	)
--	OPEN CLIENTES
...
--		VALUES (0, CURRENT_TIMESTAMP, @CLIENTE)
...
--				@ID_FACTURACION,
--				VIAJ_ID,
--				(TURN_PRECIO_BASE + TURN_VALOR_KILOMETRO * VIAJ_DISTANCIA)
--			FROM GESTION_DE_GATOS.VIAJE
--				JOIN GESTION_DE_GATOS.TURNO on VIAJ_TURNO = TURN_ID
--			WHERE CAST(VIAJ_FECHA_FIN AS DATE) = @FECHA_FACTURACION
--				AND VIAJ_CHOFER = @CLIENTE
--		)
--
--		UPDATE GESTION_DE_GATOS.FACTURACION
--		SET FACT_IMPORTE = (
...
GO
CREATE TRIGGER GESTION_DE_GATOS.validar_turno_trigger ON GESTION_DE_GATOS.TURNO
...
GO
CREATE TRIGGER GESTION_DE_GATOS.validar_viaje_trigger ON GESTION_DE_GATOS.VIAJE
INSTEAD OF INSERT
AS
BEGIN
	IF EXISTS
		...
		ON
			...
			ins.VIAJ_TURNO = viaj.VIAJ_TURNO AND
	...
	BEGIN
		RAISERROR('El viaje ya existe', 16, 1)
		ROLLBACK TRANSACTION
		RETURN
	END


	INSERT INTO GESTION_DE_GATOS.VIAJE (VIAJ_CHOFER, VIAJ_CLIENTE, VIAJ_DISTANCIA, VIAJ_FECHA_FIN, VIAJ_FECHA_INICIO, VIAJ_TURNO, VIAJ_VEHICULO)
	SELECT VIAJ_CHOFER, VIAJ_CLIENTE, VIAJ_DISTANCIA, VIAJ_FECHA_FIN, VIAJ_FECHA_INICIO, VIAJ_TURNO, VIAJ_VEHICULO
	FROM inserted ins
END
GO
