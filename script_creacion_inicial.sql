...
AS
	...
	(
		SELECT CONVERT(CHAR(255), Chofer_Dni), (SELECT GESTION_DE_GATOS.encriptar_contrasenia(RTRIM(CONVERT(CHAR(255), Chofer_Dni)))), CAST(1 AS BIT)
		FROM gd_esquema.Maestra
		GROUP BY Chofer_Dni
	)
	...
	INSERT GESTION_DE_GATOS.USUARIO (USUA_USERNAME, USUA_CONTRASENIA, USUA_HABILITADO)
	(
		SELECT CONVERT(CHAR(255), Cliente_Dni), (SELECT GESTION_DE_GATOS.encriptar_contrasenia(RTRIM(CONVERT(CHAR(255), Cliente_Dni)))), CAST(1 AS BIT)
		FROM gd_esquema.Maestra
...
AS
	...
	BEGIN
		INSERT GESTION_DE_GATOS.RENDICION (REND_FECHA, REND_IMPORTE) VALUES (@FECHA_FACTURACION, 0)
...
--	WHILE @@FETCH_STATUS = 0
--	BEGIN
--		INSERT GESTION_DE_GATOS.FACTURACION (FACT_IMPROTE, FACT_FECHA, FACT_FECHA_INICIO, FACT_FECHA_FIN)
