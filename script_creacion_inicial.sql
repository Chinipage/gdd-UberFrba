...
AS
	...
	BEGIN TRY
		...
		SET
			USUA_USERNAME = @DNI,
			USUA_CONTRASENIA = (SELECT GESTION_DE_GATOS.f_encriptar_contrasenia(@DNI))
		WHERE USUA_ID = (SELECT CLIE_USUARIO FROM GESTION_DE_GATOS.CLIENTE WHERE CLIE_ID = @CLIE_ID)
	END TRY
	BEGIN CATCH
		DECLARE
			@MESSAGE NVARCHAR(4000),
...
AS
	...
	BEGIN TRY
		...
		SET
			USUA_USERNAME = @DNI,
			USUA_CONTRASENIA = (SELECT GESTION_DE_GATOS.f_encriptar_contrasenia(@DNI))
		WHERE USUA_ID = (SELECT CHOF_USUARIO FROM GESTION_DE_GATOS.CHOFER WHERE CHOF_ID = @CHOF_ID)
...
GO
ALTER TRIGGER GESTION_DE_GATOS.t_rol ON GESTION_DE_GATOS.ROL
AFTER UPDATE
AS
BEGIN
	BEGIN TRY
